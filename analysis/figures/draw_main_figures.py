...
ax.text(.85,4.82,'GPU: B bloques independientes; una trayectoria por bloque',fontweight='bold',fontsize=9,color=BLUE)
box(.95,3.64,2.1,.64,'Selección actual\nMejor selección\nFase y hash',size=8.3)
box(3.57,3.57,5.66,.82,'',size=9)
ax.text(4.98,3.98,'Construcción\nAñadir ítem factible\nPuntuación aleatoria',ha='center',va='center',fontsize=8.8,color=BLUE)
ax.text(7.84,3.98,'Modo tabú\nAdiciones, eliminaciones\ne intercambios',ha='center',va='center',fontsize=8.3,color=BLUE)
ax.plot([6.40,6.40],[3.66,4.30],color=BLUE,lw=.7,ls=':')
ax.text(6.40,4.55,'Decidir según la fase',ha='center',fontsize=8,color=BLUE)
arrow((3.13,3.98),(3.49,3.98))
box(3.57,2.31,5.66,.72,'Hilos cooperan para evaluar ganancias exactas\nSeleccionar movimiento y actualizar cobertura,\nmemoria, incumbente y estancamiento',size=8.5)
arrow((4.74,3.53),(4.74,3.11));arrow((7.94,3.53),(7.94,3.11))
box(.95,2.31,2.10,.72,'Reinicio vacío\no perturbación de\nla incumbente',size=8.3)
arrow((3.49,2.66),(3.13,2.66));ax.text(3.30,3.13,'Estancamiento\no sin admisibles',ha='center',fontsize=7.3,color=BLUE)
arrow((2.0,3.11),(2.0,3.56))
arrow((9.29,2.64),(9.29,4.44),rad=.2);arrow((9.29,4.44),(9.05,4.44))

arrow((5.3,5.32),(5.3,5.04))
box(3.50,.72,3.63,.60,'Sincronizar resúmenes\nObservar y registrar\nla mejor solución entre bloques',size=8.2)
arrow((5.3,1.60),(5.3,1.40))
box(7.72,.72,2.48,.60,'Límite alcanzado\nCertificar y devolver\nla mejor solución',size=8.3)
