...
for ax,(g,title) in zip(axes,groups):
    ...
    ax.axvline(0,color='#888888',lw=.7,ls=':');ax.grid(axis='x',alpha=.18);ax.set_title(title+'\n30 instances',fontsize=10)
    ax.set_xlabel('Gap to benchmark best (%)',color=BLUE,fontsize=8)
    ax.set_yticks(range(1,6));ax.set_yticklabels([names[a] for a in alg]);ax.set_ylim(5.6,.4)
fig.suptitle('How does quality compare with the reference methods?',fontsize=11,color=BLUE,x=.06,ha='left')
fig.text(.06,.90,'R = maximum of literature reference and GPU-MTS best result.',fontsize=8.5,color=BLUE)
