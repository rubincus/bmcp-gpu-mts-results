...
fig.suptitle('When do differences between seeds disappear?',fontsize=12,color=BLUE,x=.03,ha='left')
...
labels=['Original','No local search','No tabu filter','No exchanges','Strict improvements only','Perturbed incumbent only','Empty restart only'];loss=np.zeros((7,6));hits=np.zeros((7,6),int)
...
cax=fig.add_axes([.89,.17,.025,.62]);cb=fig.colorbar(im,cax=cax);cb.set_label('Mean loss (%)',fontsize=8);cb.ax.tick_params(labelsize=7)
fig.suptitle('Which components sustain quality and reliability?',x=.03,ha='left',fontsize=11.8)
fig.text(.03,.88,'Each cell: hits/10 and mean percentage loss (including zero).',fontsize=8.5,color=BLUE)
fig.text(.27,.035,'D1–D2: 180 s; D3–D6: 90 s. Ten runs per cell.',fontsize=8)
...
ax.text(.2,6.42,'GPU-MTS: search organization',fontsize=12,fontweight='bold',color=BLUE)
...
ax.text(.70,.15,'Blocks retain state between batches; they do not share solutions.',fontsize=8.2,color=BLUE)
